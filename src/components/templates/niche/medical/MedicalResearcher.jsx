                ...
                <div className="col-span-8 space-y-12">
                    ...
                    {projects && projects.length > 0 && (
                        <section className="space-y-10 mt-16">
                            <h2 className="text-[10px] font-black uppercase tracking-[0.6em] text-slate-300">Featured_Publications_&_Investigative_Work</h2>
                            <div className="space-y-10">
                                {projects.map((proj) => (
                                    <div key={proj.id} className="p-10 bg-white border border-slate-100 rounded-lg shadow-lg group hover:border-slate-900 transition-colors">
                                        <div className="flex justify-between items-center mb-4">
                                            <h3 className="text-2xl font-black italic tracking-tighter uppercase text-slate-900">{proj.name}</h3>
                                            <span className="text-[10px] font-black text-slate-300 uppercase tracking-widest">{proj.techStack}</span>
                                        </div>
                                        <p className="text-xs text-slate-400 mb-4 font-bold border-b border-slate-50 pb-2 inline-block">{proj.link}</p>
                                        <p className="text-lg leading-relaxed font-serif italic text-slate-600">"{proj.description}"</p>
                                    </div>
                                ))}
                            </div>
                        </section>
